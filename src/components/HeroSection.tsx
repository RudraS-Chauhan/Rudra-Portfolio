...
export const HeroSection: React.FC<HeroSectionProps> = ({ onContactClick, onAiClick }) => {
  ...
  return (
    <section className="relative h-screen min-h-[600px] sm:min-h-[700px] w-full flex flex-col justify-between overflow-x-clip bg-[#0C0C0C] select-none">
      {/* 1. Navbar */}
      <motion.nav
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7, delay: 0, ease: [0.25, 0.1, 0.25, 1] }}
        className="w-full flex justify-between items-center px-5 sm:px-8 md:px-10 pt-6 md:pt-8 z-30 relative"
      >
        <button
        ...
        {onAiClick && (
          <button
            onClick={onAiClick}
            className="text-[#B600A8] font-semibold uppercase tracking-wider text-sm md:text-lg lg:text-[1.4rem] transition-all duration-200 hover:opacity-80 hover:scale-105 cursor-pointer flex items-center gap-1.5"
          >
      ...
      {/* 2. Hero Heading */}
      <div className="w-full overflow-hidden flex justify-center z-0 relative">
        <motion.h1
      ...
      {/* 3. Bottom bar */}
      <div className="w-full flex justify-between items-end pb-7 sm:pb-8 md:pb-10 px-5 sm:px-8 md:px-10 z-20 relative">
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
